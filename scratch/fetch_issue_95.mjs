async function run() {
  try {
    const res = await fetch("https://github.com/dianatorcates-commits/savr-app/issues/95");
    ...
    if (descMatch) {
      ...
      fs.writeFileSync('scratch/issue_95.html', html);
      console.log("Saved HTML to scratch/issue_95.html");
    }
  } catch (err) {
    console.error(err);
  }
