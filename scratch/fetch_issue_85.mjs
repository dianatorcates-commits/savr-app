async function run() {
  try {
    const res = await fetch("https://github.com/dianatorcates-commits/savr-app/issues/85");
    ...
    if (descMatch) {
      ...
      fs.writeFileSync('scratch/issue_85.html', html);
      console.log("Saved HTML to scratch/issue_85.html");
    }
  } catch (err) {
    console.error(err);
  }
